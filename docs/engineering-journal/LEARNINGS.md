...
**Context.** While unblocking CAMPPS C0.1 nonprod service deploys, the tenant setup and identity access GitHub deploy roles still denied direct reads of the shared platform SSM namespace after the main `infiquetra-aws-infra` workflow had gone green.

**Evidence.** Commit `7fb7915` updated `infiquetra_aws_infra/campps_deploy_roles_stack.py` and `tests/unit/test_campps_deploy_roles_stack.py`. GitHub Actions run `27707444975` completed successfully for `Deploy Infrastructure`, but the live tenant setup data policy remained `v1` and `iam simulate-principal-policy` still returned `implicitDeny` for `ssm:GetParametersByPath` on `arn:aws:ssm:us-east-1:477152411873:parameter/campps/platform/nonprod*`. A direct CloudFormation deploy of `CamppsNonProdDeployRolesStack` then completed `UPDATE_COMPLETE` at `2026-06-17T17:31:09.999000+00:00`, changed `campps-tenant-setup-nonprod-gha-data-policy` to `v2`, and simulation returned `allowed` for both tenant setup and identity access on `GetParameter`, `GetParameters`, and `GetParametersByPath` for the shared platform namespace.

**Mechanism.** The default foundation workflow deploys `app.py` management-account stacks (`InfiquetraOrganizationStack`, `InfiquetraSSOStack`) only. CAMPPS workload deploy roles are generated by the separate `app_campps_bootstrap.py` entry point, so source changes to those role policies are not applied to `campps-nonprod` until `CamppsNonProdDeployRolesStack` is deployed. The previous platform SSM path-read fix also covered the `platform-foundation` profile only; service repos use the `serverless-api` profile.

**Fix.** Added a serverless shared-platform SSM read grant scoped to `/campps/platform/<env>` and deployed `CamppsNonProdDeployRolesStack` directly with the `campps-nonprod` profile using the CDK bootstrap S3 bucket.

**Validation.** `uv run pytest tests/unit/test_campps_deploy_roles_stack.py -q`, `uv run ruff check infiquetra_aws_infra/campps_deploy_roles_stack.py tests/unit/test_campps_deploy_roles_stack.py`, commit hooks (`ruff`, `ruff format`, `mypy`, `bandit`), GitHub Actions run `27707444975`, CloudFormation `UPDATE_COMPLETE`, and IAM simulation for both `campps-tenant-setup-nonprod-gha-deploy-role` and `campps-identity-access-nonprod-gha-deploy-role`.

**Generalizable rule.** For this repository, a green `Deploy Infrastructure` run proves the management foundation only. Any CAMPPS workload-account deploy-role policy change must be followed by an explicit `app_campps_bootstrap.py` deployment of the relevant workload stack and a live IAM simulation against the generated service role.
