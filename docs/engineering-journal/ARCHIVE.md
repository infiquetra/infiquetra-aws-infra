...
**Shipped.** `fix/canary-invoke-function`, infiquetra-aws-infra, 2026-08-21.

The queued trigger fired exactly as written: `CamppsNonProdDeployRolesStack` was deployed with
`campps-platform-nonprod-gha-e2e-canary-health-policy`, `simulate-principal-policy` returned `allowed`
for `lambda:InvokeFunctionUrl`, and the campps-platform e2e canary still returned **HTTP 403 Forbidden**
rather than executing green — campps-platform deploy run 32433395232, 2026-08-21.

**Evidence for the diagnosis.** Simulating `lambda:InvokeFunction` with the runtime context key
`lambda:InvokedViaFunctionUrl=true` separates the two roles cleanly:

| role | `InvokeFunctionUrl` | `InvokeFunction` via URL | live |
|---|---|---|---|
| `campps-e2e-canary-nonprod-gha-deploy-role` | allowed | **allowed** | works |
| `campps-platform-nonprod-gha-deploy-role` | allowed | **implicitDeny** | 403 |

The canary's own role draws both from the function's *resource* policy, which grants them only to that
role. The platform role held the identity-side `InvokeFunctionUrl` alone.

**Original queued rationale, preserved:**
