...
### CAMPPS workload deploy roles are NOT deployed by CD

**CRITICAL**: `deploy-infrastructure.yml` deploys only the management `app.py` stacks
(`InfiquetraOrganizationStack`, `InfiquetraSSOStack`). The CAMPPS workload deploy roles are generated by
the separate `app_campps_bootstrap.py` entry point, which **no workflow runs**. A registry or role-policy
change can merge to `main` with all CI green and apply **nothing** in the workload accounts until someone
runs a privileged `cdk deploy <CamppsNonProd|Staging|Production>DeployRolesStack --app "python3
app_campps_bootstrap.py"` against that account with AdministratorAccess-class creds.

A green `Deploy Infrastructure` run proves the management foundation **only** — never assume a merged
CAMPPS role change is live in AWS without an explicit bootstrap-app deploy plus a live `aws iam get-role`
/ `iam simulate-principal-policy` check. This gap bit C0.1 (2026-06-17) and C0.3 (2026-06-20). See
`docs/engineering-journal/LEARNINGS.md` (2026-06-17 / 2026-06-20) and `QUEUED.md` ("Wire the CAMPPS
bootstrap app into a guarded CD path").
...
- `campps-nonprod` (477152411873) — `Apps/CAMPPS/NonProd` (`ou-f3un-yb8hu7vq`)
...
Both accounts inherit `BaseSecurityPolicy` from the `Apps` OU; `campps-nonprod` additionally inherits `NonProductionCostControl` from `NonProd` (restricts EC2 launches to t3/t4g nano-medium).
